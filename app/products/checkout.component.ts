      ...
      data => 
      {
        ...
        if(this.todoservice.get_param('tracker') && this.todoservice.get_param('tracker') == 'favorite')
        {
          ...
          if(data.favourites.items.length > 0)
          {
            ...
            var q = 1;
            if(myitems[i].quantity)
              q = myitems[i].quantity;

            this.product_items.push({product: myitems[0].product,quantity : q});
          } 
        }
        else if(this.todoservice.get_param('tracker') && this.todoservice.get_param('tracker') == 'cart')
        { 
          ...
          if(data.favourites.items.length > 0)
          {
           ...
           // console.log(myitems);
            ...
            for(var i = 0;i < myitems.length ; i++)
            {
              ...
              if(myitems[i].quantity)
                q = myitems[i].quantity;
              this.product_items.push({product : myitems[i].product,quantity : q});
            }
          }
        }
...
changeItemCount(id :number, op : string, product : any )
{
...
add_to_favorite(product : any)
{
  ...
  this.spinner.show() 
  this.todoservice.add_to_favorite({product : product,type : this.todoservice.get_param('tracker'),token : this.get_token()})
  .subscribe(
  data => 
  {
    this.spinner.hide();
