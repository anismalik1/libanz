...
@Component({
  ...
  templateUrl: './book-dth-order.component.html',
  styles: []
})
...
get_categories()
{
  ...
  this.todoservice.fetch_product_categories(data)
    .subscribe(
      data => 
      {
        if(data.status == 'Invalid Token')
        ...
        this.spinner.hide();
        this.quallitydisabled = false;
      }
    )  
}

call_dth_quality(event)
{
  this.spinner.show();
  let data : any = {token : this.get_token(),catid:event.value,subid:$('#subid').val()};
  this.todoservice.fetch_product_quality(data)
    .subscribe(
      data => 
      {
        if(data.status == 'Invalid Token')
        ...
        if(!jQuery.isEmptyObject(data))
        {
          this.categories_quality = data.products;
        }
        this.spinner.hide();
      }
    )  
